// krx_fix_pipeline_test.cc -- Verifies the full KRX order entry pipeline:
//
//   IceCodec (encode FIX NOS) -> KrxFixGateway (decode + dispatch)
//                              -> KrxSimulator (matching engine)
//                              -> KrxFixExecPublisher (encode FIX response)
//                              -> IceCodec (decode FIX response)
//
// This test reproduces the exact flow from the smoke test:
//   exchange-trader --exchange krx -> krx-sim -> exchange-observer
//
// The root cause of the pipeline failure was that krx_sim_runner.cc did not
// call exec_pub.register_order() before dispatching orders to the engine.
// Without registration, the exec publisher silently dropped all events,
// producing no FIX execution reports back to the trader.
...
struct TestFixEncoder {
    ::exchange::ice::fix::EncodeContext ctx;
    std::string symbol;
    ...
    // Encode a NewOrderSingle (35=D), same as IceCodec::encode_new_order.
    // account: numeric account ID for FIX tag 1.  KRX uses account_id for
    // SMP (Self-Match Prevention), so crossing orders must use different
    // accounts.
    ...
    // Encode a CancelRequest (35=F), same as IceCodec::encode_cancel.
...
// Decode a FIX exec report using the same logic as IceCodec::decode_response.
...
// ---------------------------------------------------------------------------
// Fixture: wires up the same pipeline as krx_sim_runner.cc.
// ---------------------------------------------------------------------------

class KrxFixPipelineTest : public ::testing::Test {
protected:
    using Sim = KrxSimulator<KrxFixExecPublisher, fast::FastFeedPublisher>;

    KrxFixExecPublisher exec_pub_{"KRX-SIM", "CLIENT", "KS"};
    fast::FastFeedPublisher fast_pub_;
    Sim sim_{exec_pub_, fast_pub_};

    // SymbolRouter: same pattern as krx_sim_runner.cc
    struct SymbolRouter {
        Sim& sim;
    ...
    KrxFixGateway<SymbolRouter> gateway_{router_};

    // Symbol -> instrument_id map
    std::unordered_map<std::string, uint32_t> symbol_map_;

    // FIX encoder (mimics IceCodec in exchange-trader)
    TestFixEncoder encoder_{"KS", "FIRM_A", "EXCHANGE"};
    ...
    void SetUp() override {
        ...
        // Set reference prices so VI/band checks don't reject orders.
        // KS (KOSPI200) at 350.0000 = 3500000 fixed-point.
        sim_.set_prices(KS_ID, 3500000, 3500000);

        // Open the market for continuous trading.
        sim_.start_regular_session(1000);
        sim_.open_regular_market(1100);
        exec_pub_.clear_messages();
    }

    // Simulate the on_message handler from krx_sim_runner.cc:
    ...
    bool dispatch_fix(const std::string& fix_msg, Timestamp ts = 2000) {
        ...
        // Register order for new orders (35=D) -- the critical fix.
        if (msg.msg_type == "D") {
...
// ===========================================================================
// Test 1: Single order accepted -- the core failure scenario.
//
// This is the exact path that was broken: trader sends NOS via IceCodec,
// gateway decodes it, engine processes it, exec publisher should produce
// an ExecutionReport(ExecType=0/New) back. Without register_order(),
// the exec publisher silently dropped the event and no ack was sent.
// ===========================================================================

TEST_F(KrxFixPipelineTest, SingleOrderAccepted) {
    std::string nos = encoder_.encode_new_order(
        1, Side::Buy, 3500000, 10000);
...
TEST_F(KrxFixPipelineTest, CrossingOrdersProduceFill) {
    // Resting sell at 350.00 (account=1)
    std::string sell = encoder_.encode_new_order(
        10, Side::Sell, 3500000, 10000, /*account=*/1);
    ASSERT_TRUE(dispatch_fix(sell, 2000));
    ASSERT_EQ(exec_pub_.messages().size(), 1u);
    exec_pub_.clear_messages();

    // Aggressive buy at 350.00 (account=2, different from sell to avoid SMP).
    std::string buy = encoder_.encode_new_order(
        20, Side::Buy, 3500000, 10000, /*account=*/2);
...
TEST_F(KrxFixPipelineTest, SymbolRoutesToCorrectInstrument) {
    std::string nos = encoder_.encode_new_order(
        1, Side::Buy, 3500000, 10000);
    ASSERT_TRUE(dispatch_fix(nos, 2000));
    ...
    EXPECT_EQ(engine->active_order_count(), 1u)
        << "Order must be routed to KOSPI200 (instrument_id=1)";
}
...
TEST_F(KrxFixPipelineTest, UnknownSymbolRejected) {
    TestFixEncoder bad_encoder{"FAKE", "FIRM_A", "EXCHANGE"};
...
TEST_F(KrxFixPipelineTest, CancelOrderThroughPipeline) {
    std::string nos = encoder_.encode_new_order(
        1, Side::Buy, 3500000, 10000);
    ASSERT_TRUE(dispatch_fix(nos, 2000));
    ASSERT_EQ(exec_pub_.messages().size(), 1u);
...
// ===========================================================================
// Test 6: Multiple orders from different traders.
// ===========================================================================

TEST_F(KrxFixPipelineTest, MultipleTraders) {
    TestFixEncoder trader1{"KS", "TRADER_1", "EXCHANGE"};
    TestFixEncoder trader2{"KS", "TRADER_2", "EXCHANGE"};

    std::string sell = trader1.encode_new_order(
        1, Side::Sell, 3500500, 10000);
    ASSERT_TRUE(dispatch_fix(sell, 2000));

    std::string buy = trader2.encode_new_order(
        2, Side::Buy, 3500000, 10000);
    ASSERT_TRUE(dispatch_fix(buy, 2001));

    ASSERT_EQ(exec_pub_.messages().size(), 2u);

    TestExecReport r1, r2;
    ASSERT_TRUE(decode_exec_report(exec_pub_.messages()[0], r1));
    ASSERT_TRUE(decode_exec_report(exec_pub_.messages()[1], r2));
    EXPECT_EQ(r1.type, TestExecReport::Accepted);
    EXPECT_EQ(r2.type, TestExecReport::Accepted);
